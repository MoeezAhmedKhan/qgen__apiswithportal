...
if(isset($_GET['FunctionName'])){
 if($_GET['FunctionName']=="ToggleCampaignPro"){
 ...
 }else if ($_GET['FunctionName']=="DeleteAnnouncement"){
     $id = $_GET['id'];
...
}else if(isset($_POST['BtnUopdateclientStatus'])){
      include('connection.php');
      $id =  $_POST['id'];
      $name =  $_POST['name'];
      $email =  $_POST['email'];
    ...
    //   die();
      ...
      $client_sql =  "UPDATE `clients` SET `full_name`= '$name',`email`= '$email',`status`= '$status' WHERE `id`= '$id'";
      $client_result = mysqli_query($conn,$client_sql);
      if($client_result)
      {
        header('Location: ../view_client.php?Message=Sucessfully updated client');
      }
      else
      {
          echo "error";
...
// 
    ...
    //   echo 1;
      ...
      $sql = "UPDATE `users` SET `fullname`='$name' ,`email`='$email' ,`Status`='$status' WHERE `id`= '$id'";
      
      $result = mysqli_query($conn,$sql);
      if($result){
        header('Location: ../view_user.php?Message=Sucessfully updated details.');
      }else{
          echo "error";
      }
...
else if(isset($_POST['btnUpdateAnnouncement'])){
      include('connection.php');
    //   echo 1;
      $id =  $_POST['id'];
      $title =  $_POST['title'];
      ...
      $sql = "UPDATE `sliders` SET `title`='$title',`description`='$descr' WHERE `id` ='$id'";
      
      $result = mysqli_query($conn,$sql);
      if($result){
        header('Location: ../manageAnnouncements.php?Message=Sucessfully updated announcements.');
      }
...
else if(isset($_POST['btnUpdaterevenue'])){
      include('connection.php');
    //   echo 1;
      $id =  $_POST['name'];
      $rev =  $_POST['email'];
      ...
      $sql = "UPDATE `revenue` SET `total_revenue`= '$rev' ,`revenue_last_given`= '$rev_time' WHERE `user_id`= '$id'";
      
      $result = mysqli_query($conn,$sql);
      if($result){
        header('Location: ../view_transactions.php?Message=Sucessfully updated details.');
      }
      else
      {
